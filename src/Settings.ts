...
import { resolveTFolder, arraymove } from "Utils";
    ...
    displayPreambleToggles(): void {
        ...
        this.plugin.settings.preambles.forEach((preamble, index) => {
            const s = new Setting(this.containerEl)
                .setName("Preamble")
                .addSearch(cb => {
                    const preambleTFolder = resolveTFolder(this.app, this.plugin.settings.preambleFolder);

                    new FileSuggest(this.app, cb.inputEl, this.plugin, preambleTFolder, ["sty"]);
                    cb
                        .setPlaceholder("Preamble file")
